...
def variable_parties() -> dict[str:BaseParty]:
    parties:dict[str:BaseParty] = {}
    folder = r"data/characters/parties/"
    files = os.listdir(folder)
    print(files)

    for file in files:
        data = saves.Saves(folder + file).loaded_data
        parties[data["name"]] = BaseParty(
            data["name"],
            data["leader"],
            data["participants"],
            pol_com.PoliticalCompass(
                data["political_ideas"]["left-right"],
                data["political_ideas"]["freedom"]
            )
        )
